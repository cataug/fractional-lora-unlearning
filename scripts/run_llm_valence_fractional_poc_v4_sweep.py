...
OUT_ROOT = ROOT / "llm_valence_fractional_poc_v4_sweep"
...
def build_scenarios() -> List[Dict[str, Any]]:
    ...
    scenarios.append({
        "scenario_id": "B1",
        "scenario_name": "valence_frac_lora_replace_a050",
    ...
    # B4 alpha/lambda sweep — main hypothesis
    for alpha in [0.40, 0.50, 0.60, 0.70]:
        for lam in [0.005, 0.010, 0.020, 0.050]:
            scenarios.append({
                "scenario_id": f"B4_a{str(alpha).replace('.', '')}_l{str(lam).replace('.', '')}",
    ...
    # Full/global negative control — only seed 42 in grid construction.
    scenarios.append({
        "scenario_id": "C4",
        "scenario_name": "valence_frac_all_a050_negative_control",
        "method": "valence_fractional",
        "trainable_scope": "all",
        "target": "all",
        "alpha": 0.50,
...
def load_taught_model_trainable(model_cfg: Dict[str, Any], seed: int, device: torch.device) -> nn.Module:
...
def load_taught_model_frozen(model_cfg: Dict[str, Any], seed: int, device: torch.device) -> nn.Module:
    base = load_base_model(model_cfg, device)
    model = PeftModel.from_pretrained(
        base,
        get_teach_dir(model_cfg, seed),
...
def train_teach_stage(
    ...
    if RESUME and teach_checkpoint_exists(model_cfg, seed):
        print(f"Teach checkpoint exists for {model_cfg['model_name']} seed={seed}, skipping:", teach_dir)
        return
...
def run_one_scenario(
    ...
    result = {
        ...
        "negative_control": scenario.get("negative_control", False),
...
def save_live_summary(rows: List[Dict[str, Any]]) -> None:
    ...
    csv_path = REPORT_DIR / "poc_v4_live_summary.csv"
    json_path = REPORT_DIR / "poc_v4_live_summary.json"
    xlsx_path = REPORT_DIR / "poc_v4_live_summary.xlsx"
    ...
    try:
        with pd.ExcelWriter(xlsx_path, engine="openpyxl") as writer:
            ...
            if "unlearning_score_simple" in df.columns:
                ...
                brief_cols = [c for c in brief_cols if c in ranked.columns]
                ranked[brief_cols].to_excel(writer, sheet_name="brief", index=False)
            ...
            if "model_name" in df.columns and "scenario_name" in df.columns:
                agg = (
                    ...
                    .agg({
                        "before_retain_em_acc": ["count", "mean", "std"],
                        "before_forget_em_acc": ["mean", "std"],
                        "after_retain_em_acc": ["mean", "std"],
                ...
                global_agg.to_excel(writer, sheet_name="global_scenario_mean", index=False)
...
def precheck() -> None:
    print("=" * 100)
    print("LLM VALENCE FRACTIONAL POC V4 — MIX SWEEP")
    print("=" * 100)
    ...
    for s in SCENARIOS:
        print(
            ...
            "alpha=", s["alpha"],
            "lambda=", s["mix_lambda"],
            "negative=", s.get("negative_control", False),
        )
    ...
    for model_cfg in MODEL_CONFIGS:
        ...
        for seed in RUN_SEEDS:
            ...
            if metrics.get("retain_em_acc", 0.0) < 0.40 or metrics.get("forget_em_acc", 0.0) < 0.40:
                print("\nWARNING:")
                print(f"{model_cfg['model_name']} seed={seed} teach exact-match is still low.")
    ...
    for model_cfg in MODEL_CONFIGS:
        for seed in RUN_SEEDS:
            for scenario in SCENARIOS:
                if scenario.get("negative_control", False) and seed != 42:
                    continue
...
def main() -> None:
    ...
    print("RUN_DIR:", RUN_DIR)
    print("REPORT_DIR:", REPORT_DIR)
    print("Summary CSV:", REPORT_DIR / "poc_v4_live_summary.csv")
    print("Summary XLSX:", REPORT_DIR / "poc_v4_live_summary.xlsx")
