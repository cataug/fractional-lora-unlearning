...
OUT_ROOT = ROOT / "llm_valence_fractional_poc_v2"
...
# This is v2: stronger teach-stage.
N_RETAIN_FACTS = 100
N_FORGET_FACTS = 100
N_GENERAL_TEXTS = 100
...
RUN_SEEDS = [42]
# For stronger repeatability later:
# RUN_SEEDS = [42, 43, 44]


# ============================================================
# PARALLEL / GPU SCHEDULER CONFIG
# ============================================================
...
def load_taught_model_trainable(device: torch.device) -> nn.Module:
...
def load_taught_model_frozen(device: torch.device) -> nn.Module:
    base = load_base_model(device)
    model = PeftModel.from_pretrained(
        base,
        TEACH_DIR,
...
def train_teach_stage(
    ...
    if RESUME and teach_checkpoint_exists():
        print("Teach checkpoint exists, skipping:", TEACH_DIR)
        return

    print("\n" + "=" * 100)
    print("TEACH STAGE V2")
    print("=" * 100)
    ...
    for epoch in range(1, TEACH_EPOCHS + 1):
        ...
        log(
            f"Teach epoch {epoch}/{TEACH_EPOCHS} | "
...
# ============================================================
# TRAIN FRACTIONAL SCENARIOS — FIXED, NO GRADSCALER
# ============================================================
...
def scenario_out_dir(scenario: Dict[str, Any], run_seed: int) -> Path:
    run_name = f"{scenario['scenario_id']}_{safe_name(scenario['scenario_name'])}_seed{run_seed}"
...
def run_one_scenario(
    ...
    out_dir = scenario_out_dir(scenario, run_seed)
    ...
    paths = {
        "retain": DATA_DIR / "retain_facts.jsonl",
        "forget": DATA_DIR / "forget_facts.jsonl",
        "general": DATA_DIR / "general_texts.jsonl",
    }
    ...
    model = load_taught_model_trainable(device)
...
def evaluate_teach_if_needed() -> Dict[str, Any]:
    ...
    device = DEVICE_MAIN
    model = load_taught_model_frozen(device)
...
def collect_existing_results() -> List[Dict[str, Any]]:
    ...
    for done_path in sorted(RUN_DIR.glob("*/*")):
        pass
...
def save_live_summary(rows: List[Dict[str, Any]]) -> None:
    ...
    csv_path = REPORT_DIR / "poc_v2_live_summary.csv"
    json_path = REPORT_DIR / "poc_v2_live_summary.json"
    xlsx_path = REPORT_DIR / "poc_v2_live_summary.xlsx"
...
def precheck() -> None:
    print("=" * 100)
    print("LLM VALENCE FRACTIONAL POC V2 PARALLEL")
    print("=" * 100)
    print("ROOT:", ROOT)
    print("BASE_MODEL_DIR:", BASE_MODEL_DIR)
    print("OUT_ROOT:", OUT_ROOT)
    print("DATA_DIR:", DATA_DIR)
    print("TEACH_DIR:", TEACH_DIR)
...
def main() -> None:
    ...
    teach_metrics = evaluate_teach_if_needed()

    print("\n" + "=" * 100)
    print("TEACH METRICS")
    print("=" * 100)
    print(json.dumps(teach_metrics, indent=4, ensure_ascii=False))

    if teach_metrics.get("retain_em_acc", 0.0) < 0.20 or teach_metrics.get("forget_em_acc", 0.0) < 0.20:
        print("\nWARNING:")
        print("Teach generation exact-match is still low.")
        print("Unlearning can still be evaluated by NLL, but exact-match forgetting may be weak.")
        print("If needed later, increase TEACH_EPOCHS or reduce number of facts.")
    ...
    with ProcessPoolExecutor(max_workers=MAX_PARALLEL_JOBS_HARD_CAP) as executor:
        while pending_jobs or running:
            ...
            for future in done:
                ...
                # Avoid duplicates in live summary by scenario+seed.
                all_results = [
                    r for r in all_results
                    if not (
                        r.get("scenario_name") == result.get("scenario_name")
                ...
                if result.get("status") == "failed":
                    print(
                        ...
                        f"{result.get('scenario_name')} | {result.get('error')}",
                        flush=True,
                    )
    ...
    print("RUN_DIR:", RUN_DIR)
    print("REPORT_DIR:", REPORT_DIR)
    print("Summary CSV:", REPORT_DIR / "poc_v2_live_summary.csv")
    print("Summary XLSX:", REPORT_DIR / "poc_v2_live_summary.xlsx")
    ...
    if final_rows:
        ...
        if "unlearning_score_simple" in df.columns:
            ...
            print(df.sort_values("unlearning_score_simple", ascending=False)[cols].to_string(index=False))
